...
if os.name == 'posix':
    import termios, fcntl  # for getKey(), ToDo: Reprogram for Windows

commitsuicideV, showVid, vCruns, lockV, debugV = False, False, False, threading.Lock(), True  # Global variables for video-decoding
...
class Drone(object):
    ...
    def startup(self):
        ...
        try:
            socket.socket().connect((self.DroneIP, 23))
            socket.socket().close()
...
def decode_Header(data):
    ...
    stateBit[3] = data[
                      1] >> 3 & 1  # 3: CONTROL ALGO :                (0) euler angles control, (1) angular speed control
    stateBit[4] = data[
    ...
    stateBit[23] = data[
                       1] >> 23 & 1  # 23: PIC Version number OK :        (0) a bad version number, (1) version number is OK
    ...
    stateBit[29] = data[
                       1] >> 29 & 1  # 29: ADC Watchdog :                (0) uart2 is good, (1) delay in uart2 dsr (> 5ms)
...
def decode_ID0(packet):  # NAVDATA_DEMO_TAG
    ...
    demo[5] = dataset[
        11]  # num_frames                streamed frame index                     (uint32) (Not used to integrate in video stage)
...
def decode_ID4(packet):  # NNAVDATA_GYROS_OFFSETS_TAG
    ...
    gyros_offsets = [0, 0, 0]
    for i in range(0, 3, 1):        gyros_offsets[i] = dataset[
        i + 2]  # offset_g[xyz]                in deg/s                    (float)
...
def decode_ID16(packet):  # NAVDATA_VISION_DETECT_TAG
    ...
    for i in range(0, 4, 1):        vision_detect[2][i] = dataset[
        7 + i]  # xc[4]                                (uint32)
    for i in range(0, 4, 1):        vision_detect[3][i] = dataset[
        11 + i]  # yc[4]                                (uint32)
    for i in range(0, 4, 1):        vision_detect[4][i] = dataset[
        15 + i]  # width[4]                            (uint32)
    for i in range(0, 4, 1):        vision_detect[5][i] = dataset[
        19 + i]  # height[4]                            (uint32)
    ...
    for i in range(0, 4, 1):
        for j in range(0, 3, 1):    vision_detect[9][i][j] = dataset[
            67 + i + j]  # rotation[4]                        (float 3 vector)
    for i in range(0, 4, 1):        vision_detect[10][i] = dataset[
        79 + i]  # camera_source[4]                    (uint32)
...
def decode_ID19(packet):  # NAVDATA_VIDEO_STREAM_TAG
    ...
    video_stream[2] = dataset[
        4]  # frame_number    frame index                                                   (uint32)
    video_stream[3] = dataset[
        5]  # atcmd_ref_seq    atmcd ref sequence number                                   (uint32)
    ...
    video_stream[6] = dataset[
        8]  # atcmd_ref_quality        estimator of atcmd link quality                       (uint32)
    # Drone 2.0:
    video_stream[7] = dataset[9]  # out_bitrate            measured out throughput from the video tcp socket    (uint32)
    video_stream[8] = dataset[
        10]  # desired_bitrate        last frame size generated by the video encoder        (uint32)
    for i in range(0, 5, 1):        video_stream[9][i] = dataset[
        11 + i]  # data        misc temporary data                (int32)
    video_stream[10] = dataset[
        16]  # tcp_queue_level        queue usage                                            (uint32)
    video_stream[11] = dataset[
        17]  # fifo_queue_level        queue usage                                            (uint32)
    return (video_stream)
...
def decode_ID22(packet):  # NAVDATA_MAGNETO_TAG
    ...
    for i in range(0, 3, 1):        magneto[0][i] = dataset[
        2 + i]  # mx/my/mz                                            (int16)
    for i in range(0, 3, 1):        magneto[1][i] = dataset[
        5 + i]  # magneto_raw        magneto in the body frame [mG]    (vector float)
    for i in range(0, 3, 1):        magneto[2][i] = dataset[
        8 + i]  # magneto_rectified                                    (vector float)
    for i in range(0, 3, 1):        magneto[3][i] = dataset[
        11 + i]  # magneto_offset                                    (vector float)
